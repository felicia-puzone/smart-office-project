...
void loop() {
  ...
  if (buttonState == HIGH) {
    ...
    if (currentMillis - startMillis >= period)  //test whether the period has elapsed
    {
      ...
      //Siccome l'analog read legge un valore a 10 bit (0-1023), c'è bisogno di splittare l'input e ricombinarlo per l'invio su seriale

      int light_sensor_low = lowByte( light_sensor_read);
      int light_sensor_high = highByte( light_sensor_read);
